...
class TestHttpClient(object):
    ...
    def test_intraday_meta(self, mocker):
        ...
        requests.get.assert_called_once_with('https://api.fugle.tw/realtime/v0.3/intraday/meta?symbolId=2884&apiToken=demo')

    def test_intraday_quote(self, mocker):
        mocker.patch('requests.get')
        client = HttpClient()
        client.intraday.quote(symbolId = '2884')
        requests.get.assert_called_once_with('https://api.fugle.tw/realtime/v0.3/intraday/quote?symbolId=2884&apiToken=demo')

    def test_intraday_chart(self, mocker):
        mocker.patch('requests.get')
        client = HttpClient()
        client.intraday.chart(symbolId = '2884')
        requests.get.assert_called_once_with('https://api.fugle.tw/realtime/v0.3/intraday/chart?symbolId=2884&apiToken=demo')

    def test_intraday_dealts(self, mocker):
        mocker.patch('requests.get')
        client = HttpClient()
        client.intraday.dealts(symbolId = '2884', limit=50)
        requests.get.assert_called_once_with('https://api.fugle.tw/realtime/v0.3/intraday/dealts?symbolId=2884&limit=50&apiToken=demo')

    def test_intraday_volumes(self, mocker):
        mocker.patch('requests.get')
        client = HttpClient()
        client.intraday.volumes(symbolId = '2884')
        requests.get.assert_called_once_with('https://api.fugle.tw/realtime/v0.3/intraday/volumes?symbolId=2884&apiToken=demo')